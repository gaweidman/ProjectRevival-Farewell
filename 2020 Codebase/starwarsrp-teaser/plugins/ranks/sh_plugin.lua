...
ix.command.Add("CharSetRank", {
    ...
    OnRun = function(self, client, target, rank)
        ...
        local rankTable = classTable.Ranks
...
ix.command.Add("CharRaise", {
    ...
    OnRun = function(self, client, target, rank)
        ...
        --[[
        if (client:SteamID() == target:SteamID()) then 
            return client:NotifyLocalized("cannotAllowRaiseYourself") 
        end

        if (not rankTable or not rankTable[rank] or not isnumber(rank) or (rank > #rankTable)) then
            return client:NotifyLocalized("undefinedRank")
        end

        if (client:Team() ~= target:Team()) or !rankTable[client:GetCharacter():GetRank()][4] or (rank >= client:GetCharacter():GetRank()) then
            return client:NotifyLocalized("cannotAllowRaise")
        end
