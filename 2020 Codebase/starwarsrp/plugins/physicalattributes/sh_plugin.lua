...
ix.command.Add("RollStat", {
	...
	OnRun = function(self, client, stat)
		...
		if (character and character:GetAttribute(stat, 0)) then
			...
			ix.log.Add(client, "rollstat", stat, roll + bonus, roll, bonus, 100)
		end
	end
})
...
if (SERVER) then
	...
	ix.log.AddType("rollstat", function(client, attrib, total, value, bonus, max)
		return string.format("%s statrolled %s and got %d (%d + %d) out of %d.", client:Name(), total, attrib, value, bonus, max)
	end)
	...
	hook.Add("PlayerHurt", "playerMeleeDamage", function(victim, attacker, hpLeft, dmgTaken)
		local char = victim:GetCharacter()
		if (char:GetAttribute("con", 0) < 30) then
			local chance = math.random(1,500)
			if chance == 1 then
				char:UpdateAttrib("con", 0.01)
			end
		end
	end)
	...
	-- Still not sure what the agility bar is for. Hasn't caused any problems yet, so.
	--[[
	ix.bar.Add(function()
