	...
	hook.Add("InitializedConfig", "ixChatTypes", function()
		...
		ix.chat.Register("ic", {
			...
			OnChatAdd = function(self, speaker, text)
				...
				if (LocalPlayer():GetEyeTrace().Entity == speaker) then
					...
					chat.AddText(chatColor, speaker:GetCharacter():GetName().." says (to you) \"", color_white, text, chatColor, "\"")
				else
					chat.AddText(chatColor, speaker:GetCharacter():GetName().." says \"", color_white, text, chatColor, "\"")
				end
			end,
			prefix = {"//", "/OOC"},
			description = "@cmdOOC",
		...
		ix.chat.Register("w", {
			...
			OnChatAdd = function(self, speaker, text)
				...
				local chatColor = ix.config.Get("chatColor") - Color(35, 35, 35)

				chat.AddText(chatColor, " whispers \"", color_white, text, chatColor, "\"")
			end,
		})
		...
		ix.chat.Register("y", {
			...
			OnChatAdd = function(self, speaker, text)
				...
				local chatColor = ix.chat.classes.ic:GetColor(speaker, text) + Color(35, 35, 35)

				chat.AddText(chatColor, " yells \"", color_white, text, chatColor, "\"")
			end,
		})
