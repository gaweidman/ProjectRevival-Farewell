...
function PANEL:Init()
	...
	self.profilePicture = self:Add("AvatarImage")

	self.profilePicture.player = self:GetParent().player
	self.profilePicture:SetSize(64, 64)
	self.profilePicture:Dock(LEFT)
	self.profilePicture:SetPlayer(self.profilePicture.player, 64)

	self.profilePicture.DoClick = function() end	
	self.profilePicture.DoRightClick = function()
		local client = self.profilePicture.player
	...
	self.profilePicture.OnMouseReleased = function(releasedPanel, key)
		if (key == MOUSE_LEFT) then
			self.profilePicture:DoClick()
		elseif (key == MOUSE_RIGHT) then
			self.profilePicture:DoRightClick()
		end
	end

	self.profilePicture:SetHelixTooltip(function(tooltip)
		local client = self.player

		if (IsValid(self) and IsValid(client)) then
			local nameRow = tooltip:AddRow()
			nameRow:SetImportant()
			nameRow:SetText(client:SteamName())
			nameRow:SetBackgroundColor(team.GetColor(client:Team()))
			nameRow:SizeToContents()

			local currentPing = client:Ping()
			
			local pingRow = tooltip:AddRow("ping")

			pingRow:SetText(L("ping", currentPing))
			pingRow.Paint = function(_, width, height)
				surface.SetDrawColor(ColorAlpha(derma.GetColor(
					currentPing < 110 and "Success" or (currentPing < 165 and "Warning" or "Error")
				, tooltip), 22))
				surface.DrawRect(0, 0, width, height)
			end
			pingRow:SizeToContents()
