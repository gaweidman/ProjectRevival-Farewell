...
do
	...
	ix.char.RegisterVar("attributes", {
		...
		isLocal = true,
		OnDisplay = function(self, container, payload)
			local attribMaximum = 10

			local spendablePoints = 5

			if (attribMaximum < 1) then
				return
			end
			...
			totalBar:SetMax(attribMaximum)
			totalBar:SetValue(spendablePoints)
			totalBar:Dock(TOP)
			totalBar:DockMargin(2, 2, 2, 2)
			totalBar:SetReadOnly(true)
			totalBar:SetColor(Color(20, 120, 20, 255))
			totalBar:SetText(spendablePoints.." Points Left")
			...
			for k, v in SortedPairsByMemberValue(ix.attributes.list, "name") do
				...
				bar.OnChanged = function(this, difference)
					print(difference)
					if ((total + difference) > spendablePoints) then
						return false
					end
					...
					totalBar:SetText(totalBar:GetValue().." Points Left")
				end
