...
if (SERVER) then
	...
	function ENT:SpawnRation(callback, releaseDelay)
		...
		timer.Simple(releaseDelay, function()
			ix.item.Spawn("customitem", self.dummy:GetPos(), function(item, entity)
				self.dummy:SetNoDraw(true)
			end, self.dummy:GetAngles(), {name = "test", model = "test", description = "test"})
