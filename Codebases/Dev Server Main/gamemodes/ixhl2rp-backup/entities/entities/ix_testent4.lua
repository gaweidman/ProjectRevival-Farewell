...
ENT.Type = "anim"
ENT.PrintName = "Player Vending Machine"
ENT.Category = "HL2 RP"
ENT.Spawnable = true
ENT.AdminOnly = true
...
if (SERVER) then
	...
	function ENT:SpawnFunction(client, trace)
		local vendor = ents.Create("ix_testent4")
	...
	function ENT:SetID()
		self.uniqueID = math.random(1, 2048)
	
	
		for k, v in ipairs(ents.FindByClass("ix_testent4")) do
			if v != self and v.uniqueID == self.uniqueID then
				print("Reshuffling vending machine ID")
				self:SetID()
				return
			end
		end
	
		print("vending machine ID successfully set.")
	end
