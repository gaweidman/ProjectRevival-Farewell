...
ENT.Type = "anim"
ENT.PrintName = "Long Range Radio Jammer"
ENT.Category = "HL2 RP"
ENT.Spawnable = true
ENT.AdminOnly = true
...
if CLIENT then
	...
	function ENT:Draw()
		...
		local position = self:GetPos() + forward * 13 + up * 12

		local rectW = (highBound.y - lowBound.y )/ 0.1 - 174
		...
		cam.Start3D2D(position - lowBound.y*right, self:GetAngles() + Angle(0, 90, 90), 0.1)
			...
			surface.DrawRect(174/2 - 5, 20, rectW, 16)

			surface.SetDrawColor(color_white)
			surface.DrawRect(174/2 + 2 - 5, 22, batteryW, 12)

			draw.SimpleText(math.ceil(percentage*100).."%", "BudgetLabel", 174/2 + rectW/2 - 2, 20 + 12/2 + 2, HSVToColor(h, 1, 1), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		cam.End3D2D()

		render.SetMaterial(glowMaterial)
		render.DrawSprite(position + forward*0.75 + up*-0.25, 5, 5, color)
	end
end