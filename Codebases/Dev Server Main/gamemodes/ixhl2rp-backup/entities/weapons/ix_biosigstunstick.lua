...
SWEP.Author = "Chessnut, QIncarnate"
	...
	if (self.Owner:KeyDown(IN_WALK)) then
		if (SERVER) then
			...
			if (state) then
				...
				--self:GetOwner():ChatPrint("Knockout mode activated.")
				...
				--self:GetOwner():ChatPrint("Knockout mode deactivated.")
			end
	...
	if (SERVER and trace.Hit) then
		...
		if (IsValid(entity)) then
			if (entity:IsPlayer()) then
				...
				entity:ScreenFade( SCREENFADE.IN, color_white, 0.4, 0.2 )
...
function SWEP:SecondaryAttack()
	...
	if (SERVER and IsValid(entity)) then
		...
		elseif (entity:IsPlayer()) then
			local direction = self.Owner:GetAimVector() * (300 + (self.Owner:GetChar():GetAttribute("str", 0) * 3))
				direction.z = 0
...
function SWEP:Reload()
	...
	if (SERVER) then
		if self.Owner:KeyPressed(IN_RELOAD) then
			...
			if(self.Primary.Damage == 5) then
				self.Primary.Damage = 10
				self:GetOwner():ChatPrint("Set to Medium.")

			elseif(self.Primary.Damage == 10) then
				self.Primary.Damage = 20
				self:GetOwner():ChatPrint("Set to High.")

			elseif(self.Primary.Damage == 20) then
				self.Primary.Damage = 35
				self:GetOwner():ChatPrint("Set to Extreme.")

			elseif(self.Primary.Damage == 35) then
				self.Primary.Damage = 5
				self:GetOwner():ChatPrint("Set to Low.")
			end
