...
function SWEP:CanPrimaryAttack()
    local owner = self:GetOwner()
    local char = owner:GetCharacter()
    local vortessence = char:GetData("Vortessence", 200)
...
function SWEP:CanSecondaryAttack()
    local owner = self:GetOwner()
    local char = owner:GetCharacter()
    local vortessence = char:GetData("Vortessence", 0)
    local altDown = owner:KeyDown(IN_WALK)
    if self:GetVar("Attack", nil) == "HealthCharge" then
        return true
    ...
    if (isAttacking) and (animTime != -1 and curTime >= animTime) then
        if weaponMode == "Ranged" then
            ...
            elseif attack == "Zap" then
                ...
                util.ParticleTracerEx("Weapon_Combine_Ion_Cannon", lClawAttach.Pos, tr.HitPos, false, owner:EntIndex(), owner:LookupAttachment("leftclaw"))
                util.ParticleTracerEx("Weapon_Combine_Ion_Cannon", rClawAttach.Pos, tr.HitPos, false, owner:EntIndex(), owner:LookupAttachment("rightclaw"))  
            ...
            if attack == "VortiLamp" then
                ...
                if owner:GetNetVar("vortlight", nil) == nil then
                    if SERVER then
                        ...
                        vortlight:SetPos(owner:GetPos() + Vector(0, 0, 90))
...
if CLIENT then
    ...
    function SWEP:GetViewModelPosition(EyePos, EyeAng)
        ...
        self.IronSightsAng = Angle(0, 0, 0)
        self.IronSightsPos = Vector(9999, 9999, 9999)
...
function charMeta:GetVortessence()
    return self:GetData("Vortessence", 0)
end
