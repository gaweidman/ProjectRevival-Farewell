...
function SKIN:PaintInfoBar(panel, w, h, color)
	surface.SetDrawColor(color.r, color.g, color.b, 250)
	surface.DrawRect(0, 0, w, h)
end

function SKIN:PaintInfoBarBackground(panel, w, h, color)
...
function PANEL:Init()
	...
	self.button = self.display:Add("DButton")
			...
			self:CreateAnimation(0.75, {
				...
				OnComplete = function(animation, panel)
					...
					panel:CreateAnimation(0.75, {
						index = 2,
						target = {curW = 800, curH = 400},
						easing = "outCubic",

						OnComplete = function(animation, panel)
							panel.display:SetSize(400, 800)
...
function PANEL:Paint(w, h)
	...
	if self.loading then
		if self.paintRotation then
			local dispX, dispY = self.display:GetPos()
			local dispW, dispH = self.display:GetSize()
			draw.NoTexture()
	...
	local screens = {
		["mainMenu"] = function()
			...
			callCitizens.DoClick = function(panel)
				mainPanel:ShowError("[OOC] This is not implemented!")
		...
		["removeTenant"] = function()
			...
			for k, v in pairs(apartments) do
				...
				btn.DoClick = function(panel)
					...
					for index, CID in ipairs(v.tenants) do
						container.tenantList:AddOption(apartments.niceNames[index].." - "..CID, CID)
					end
			...
			container.tenantList:SetValue("Select Apartment")
			...
			submitBtn.DoClick = function()
				...
				net.Start("prAptAction")
					net.WriteUInt(APTACT_ADDTENANT, 32)
					net.WriteString(container.tenantList)
					net.WriteString(container.aptSelected and container.aptSelected.name or "")
				net.SendToServer()
			end
		end,
...
function PANEL:ShowMainMenu()
	...
	clockBtn.DoClick = function()
		...
		local timeClock = self:Add("CWUTimeclock")
...
function PANEL:Init()
	...
	self.slots = {}
	for i = 1, 8 do
		local slot = self:Add("Panel")
		slot:Dock(TOP)
		slot:DockMargin(0, 0, 0, 5)
		slot:SetTall(18)

		local label = slot:Add("DLabel")
		...
		local comboBox = slot:Add("DComboBox")
		comboBox:Dock(FILL)
		comboBox:AddChoice("Disabled")
		comboBox:SetValue("Disabled")
		comboBox:DockMargin(10, 0, 10, 0)

		function comboBox:OnSelect(index, value, data)
			stock:SetText("Stock: "..10)
		end
		
		local stock = slot:Add("DLabel")
		stock:Dock(RIGHT)
		stock:SetText("Stock: "..10)
		stock:SizeToContents()
	end

	local stockHeading = self:Add("DLabel")
	stockHeading:DockMargin(0, 10, 0, 5)
	stockHeading:SetText("Stock")
	stockHeading:Dock(TOP)
	stockHeading:SizeToContents()

	self.stockMenu = self:Add("DListView")
	
	self.stockMenu:Dock(FILL)
	local nameCol = self.stockMenu:AddColumn("Name", 1)
	local stockCol = self.stockMenu:AddColumn("Stock", 2)

	nameCol:SetWidth(310)
	stockCol:SetWidth(80)

	local stockBar = self:Add("Panel")
	stockBar:Dock(BOTTOM)
	stockBar:DockMargin(0, 5, 0, 0)
	stockBar:SetTall(22)
	
	local itemSelect = stockBar:Add("DComboBox")
	itemSelect:AddChoice("Select")
	itemSelect:SetValue("Select")
	itemSelect:Dock(FILL)
	itemSelect:DockMargin(0, 0, 10, 0)
	
	local ejectBtn = stockBar:Add("DButton")
	ejectBtn:Dock(RIGHT)
	ejectBtn:SetWide(70)
	ejectBtn:SetText("Eject")
	
	/*
	local controlHeading = self:Add("DLabel")
	controlHeading:DockMargin(0, 10, 0, 5)
	controlHeading:SetText("Controls")
	controlHeading:Dock(BOTTOM)
	controlHeading:SizeToContents()
	*/
end
...
vgui.Register("ixVendingConfig", PANEL, "EditablePanel")
...
concommand.Add("testvending", function()
	local frame = vgui.Create("DFrame")
	frame:MakePopup()
	frame:SetSize(400, 500)
	frame:Center()
	frame:SetTitle("Vending Machine Control")

	local vending = frame:Add("ixVendingConfig")
	vending:Dock(FILL)
