	...
	local subpanels = {
		[1] = {
			...
			populate = function(subpanel)
				...
				local isCitizen = self.payload.faction.index == FACTION_CITIZEN
				local hasEyes = self.payload.faction.index == FACTION_CITIZEN
				local hasSkins = self.payload.faction.index == FACTION_CITIZEN

				subpanel.model = panel:Add("ixModelPanel")
				subpanel.model:Dock(LEFT)
				subpanel.model:SetModel(self.payload.model or table.Random(self.payload.faction.models))
		
				subpanel.model:SetFOV(28)
				local lookAt = subpanel.model:GetLookAt()
				subpanel.model:SetLookAt(lookAt + Vector(0, 0, -3))
				...
				if self.payload.faction.GetDefaultName then
					local name = self.payload.faction:GetDefaultName()
				...
				if isCitizen then
					...
					local suitLbl = suitLblPanel:Add("DLabel")
					...
					local suitHelp = suitLblPanel:Add("DLabel")
					suitHelp:Dock(LEFT)
				...
				if hasEyes then
					...
					eyeColorSelect.OnMenuOpened = function(panel, dMenu)
						if IsValid(dMenu) then
							...
							dMenu.Think = function(panel)
								...
								local eyeColor = math.floor(self.payload.skin/3)
								...
								local expectedSkin = infoPanel.skinSlider:GetValue() + (eyeColor)*(math.floor((NumModelSkins(self.payload.model) + 1)/3))
					...
					eyeColorSelect.OnSelect = function(panel, index, value)
						...
						local realSkin = infoPanel.skinSlider:GetValue() + (eyeColorSelect:GetSelectedID() - 1)*(math.floor((NumModelSkins(self.payload.model) + 1)/3))
				...
				for k, v in ipairs(self.payload.faction.models or {}) do
					...
					*/
					...
					button.DoClick = function(panel)
						...
						self.payload.model = panel.model
				...
				if hasEyes then
					eyeColorSelect.Think = function(panel)
						...
						if subpanel.model.eyeCloseup and !menuOpen and !hovered then
							subpanel.model:SetCamPos(subpanel.model.oldCamPos)
							subpanel.model:SetLookAt(subpanel.model.oldLookAt)
							subpanel.model:SetModel(subpanel.model.oldModel, infoPanel.skinSlider:GetValue() + (panel:GetSelectedID() - 1)*(math.floor((NumModelSkins(self.payload.model) + 1)/3)))
							if subpanel.model.oldSkin then subpanel.model:GetEntity():SetSkin(subpanel.model.oldSkin) end
							--model:GetEntity():ResetSequence(subpanel.model.oldSequence)
					...
					infoPanel.skinSlider.OnValueUpdated = function(panel)
						subpanel.model.oldSkin = nil
						local realSkin = panel:GetValue() + (eyeColorSelect:GetSelectedID() - 1)*(math.floor((NumModelSkins(self.payload.model) + 1)/3))
						self.payload.skin = realSkin
						subpanel.model:GetEntity():SetSkin(realSkin)
					end
				end
