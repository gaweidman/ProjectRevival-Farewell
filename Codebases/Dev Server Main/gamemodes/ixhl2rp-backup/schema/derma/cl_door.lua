...
function PANEL:Init()
	...
	self.description = self:Add("DListView")
	self.description:Dock(FILL)
	self.description:AddColumn(L"name").Header:SetTextColor(Color(25, 25, 25))
	self.description:AddColumn(L"access").Header:SetTextColor(Color(25, 25, 25))
	self.description.OnClickLine = function(this, line, selected)
...
function PANEL:SetDoor(door, access, door2)
	...
	for _, v in ipairs(player.GetAll()) do
		if (v != LocalPlayer() and v:GetCharacter()) then
			self.description:AddLine(v:Name(), L(ACCESS_LABELS[access[v] or 0])).player = v
		end
	end
    ...
    self.description.Header:SetText(entity:GetNetVar("description", ""))
end
