...
function PANEL:Init()
	...
	self.label:SetFont("prMediumFont")
	...
	self.text:SetFont("prMediumLightFont")
...
AccessorFunc(PANEL, "labelPadding", "LabelPadding", FORCE_NUMBER)
AccessorFunc(PANEL, "segmented", "Segmented", FORCE_BOOL)

function PANEL:Init()
	self.labelPadding = 8

	surface.SetFont("prMenuButtonFont")
	...
	self.label:SetFont("prMenuButtonFont")
...
-- 	label:SetFont("prMenuButtonHugeFont")
...
function PANEL:CalculateAlignment(width, height, textWidth, textHeight)
	...
	local x, y
	local xPos, yPos = self:LocalToScreen(self:GetPos())
	...
	elseif (alignment <= 6) then
		local scale = math.min(width / (textWidth), 1)

		-- center
		y = height * 0.5 - scale*textHeight/2

		if self.font == "ixCharNameFont" then
...
function PANEL:Paint(width, height)
	...
	if (self.bScaleWidth) then
		...
		if (contentWidth > (width - self.padding * 2)) then
			...
			local translation = Vector(x + width * 0.5, y + self.padding - contentHeight*scale, 0)
			local matrix = Matrix()
...
function PANEL:Init()
	self.gripBar = self:Add("DButton") 
end

function PANEL:Setup()
	self.parent = self:GetParent()

	self.gripBar:SetWide(self.parent:GetCanvas():GetWide())
end

vgui.Register("prHScrollBar", PANEL, "Panel")
...
function PANEL:Init()
	...
	self.checkbox:DockMargin(2, 2, 5, 2)
	self.checkbox:SetText("")

	function self.checkbox:DoClick()
		local parent = self:GetParent()
		local newValue = !parent:GetChecked()
		if parent:CanChangeValue(newValue) then
			parent:SetChecked(newValue)
			LocalPlayer():EmitSound("weapons/ar2/ar2_empty.wav", 75, newValue and 150 or 125, 0.25)
		end
	end

	function self.checkbox:Paint(w, h)
		surface.SetDrawColor(derma.GetColor("DarkerBackground", self))
		surface.DrawRect(0, 0, w, h)
		
		if self:GetParent():GetValue() then
			surface.SetDrawColor(231, 123, 56)
			local boxInset = 3
			surface.DrawRect(boxInset, boxInset, w - boxInset*2, w - boxInset*2)
		end 
	end
...
function PANEL:LayoutElements()
	local boxSize = self:GetTall()
	self.checkbox:SetSize(boxSize - 4, boxSize - 4)
end
...
function PANEL:OnChanged(newValue)

end

function PANEL:CanChangeValue(value)
	return true
end
...
vgui.Register("prCheckboxLabel", PANEL, "EditablePanel")
