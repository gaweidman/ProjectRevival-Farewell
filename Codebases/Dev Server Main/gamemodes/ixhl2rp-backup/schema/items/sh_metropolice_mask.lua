...
ITEM.functions.Wear = {
	OnRun = function(item)
		...
		if item.player:HasBiosignal() then
...
ITEM.functions.Remove = {
	...
	OnCanRun = function(item)
		...
		return not IsValid(item.entity) and item.bBeingUsed and item.player:HasBiosignal() and item.invID == item.player:GetCharacter():GetInventory():GetID()
	end
}
