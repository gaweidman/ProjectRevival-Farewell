...
function imgui.xFillButton(x, y, w, h, bgClr, hoverClr, pressClr)
	local bw = borderWidth or 1

	local hovering = imgui.IsHovering(x, y, w, h)
	local bgColor = bgClr

	if hovering and imgui.IsPressing() then
		bgColor = pressClr
...
function imgui.xFillTextButton(text, font, x, y, w, h, textClr, bgClr, hoverClr, pressColor)
	local fgColor = textClr

	local clicked = imgui.xFillButton(x, y, w, h, bgClr, hoverClr, pressColor)
