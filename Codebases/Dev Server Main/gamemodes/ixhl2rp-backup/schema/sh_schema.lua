...
Schema.Version = "Public Build 0.5.1"
...
ix.util.Include("libs/sh_tfa.lua")
...
do
	...
	do
		...
		function CLASS:OnChatAdd(speaker, text)
			...
			local name = hook.Run("GetCharacterName", speaker, "ic")
			if speaker == LocalPlayer() then
				name = ourCharacter:GetName()
			end
	...
	ix.chat.Register("w", {
		...
		OnChatAdd = function(self, speaker, text)
			...
			local name = hook.Run("GetCharacterName", speaker, "w")
			if speaker == LocalPlayer() then
				name = ourCharacter:GetName()
			end
	...
	ix.chat.Register("y", {
		...
		OnChatAdd = function(self, speaker, text)
			...
			local name = hook.Run("GetCharacterName", speaker, "y")
			if speaker == LocalPlayer() then
				name = ourCharacter:GetName()
			end
	...
	ix.chat.Register("me", {
		...
		OnChatAdd = function(self, speaker, text)
			...
			local name = hook.Run("GetCharacterName", speaker, "me")
			if speaker == LocalPlayer() then
				name = ourCharacter:GetName()
			end
...
if SERVER then
	...
	function ix.placement.StartPlacing(ply, itemID)
		...
		PrintTable(ix.item.list[item.uniqueID])
...
ix.anim.SetModelClass("models/predatorcz/amnesia/grunt.mdl", "grief")
