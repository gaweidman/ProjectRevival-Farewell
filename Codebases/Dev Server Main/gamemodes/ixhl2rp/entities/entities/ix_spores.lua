...
if SERVER then
	...
	function ENT:Initialize()
		...
		self.sporeType = table.Random(ix.illness.sporeTypes)
		self.createTime = os.time()
	end

	function ENT:Think()
		for k, v in ents.FindInSphere(self:GetPos(), 400) do
			!v:IsPlayer() then continue end

			if v:WearingMask() then
				continue
			else
				local char = v:GetCharacter()
				if char then char:AddIllness(self.sporeType)
			end
		end
