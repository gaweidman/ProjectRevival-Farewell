...
if (SERVER) then
	...
	function ENT:Initialize()
		...
		self.dummy:SetModel("models/dorado/tarjeta3.mdl")
	...
	function ENT:SpawnRation(callback, releaseDelay)
		...
		local itemTable = ix.item.Get("uid")
		...
		timer.Simple(releaseDelay, function()
			ix.item.Spawn("uid", self.dummy:GetPos(), function(item, entity)
				self.dummy:SetNoDraw(true)
			end, self.dummy:GetAngles(), {
			description = string.format("A CWU identification card with ID #%s, assigned to %s.", cidNum, cidName) }
			)
	...
	function ENT:Use(client)
		...
		if (client:Team() == FACTION_CITIZEN or true) then
			if (!self:GetEnabled()) then
				self:DisplayError(6)
				return
			end

			cid = client:GetCharacter():GetInventory():HasItem("cid")
			cidNum = cid:GetData("id", "00000")
			cidName = cid:GetData("name", "nobody")
