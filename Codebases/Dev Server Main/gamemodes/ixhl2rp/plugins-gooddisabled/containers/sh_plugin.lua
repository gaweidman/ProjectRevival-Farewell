		...
		if (data) then
			for _, v in ipairs(data) do
				...
				if (data2) then
					...
					elseif v[6] == nil then
						entity:SetDisplayName(ix.container.stored[v[4]].name)
					end
					...
					if (v[8]) then
						entity:SetCombineLocked(true)
						entity:SetCombineRet(true)
					end

					if (v[9]) then
						entity:SetCWULocked(true)
						entity:SetCWURet(true)
					end
...
properties.Add("container_setpassword", {
	...
	Receive = function(self, length, client)
		...
		if (password:len() != 0) then
			entity:SetLocked(true)
			entity:SetCombineLocked(false)
			entity:SetCombineRet(false)

			entity:SetCWULocked(false)
			entity:SetCWURet(false)
...
properties.Add("container_makecmblocked", {
	MenuLabel = "Set Combine Locked",
	Order = 400,
	...
	Receive = function(self, length, client)
		...
		client:Notify("You have made this a combine-only container.")
		...
		ix.log.Add(client, "containerPassword", name, inventory:GetID(), true)
	end
})


properties.Add("container_makecwulocked", {
	MenuLabel = "Set CWU Locked",
	Order = 400,
	...
	Receive = function(self, length, client)
		...
		client:Notify("You have made this a CWU-only container.")
