...
function PANEL:PerformLayout(width, height)
	...
	if ix.option.Get("useAltChatFont", false) then
		self:SetTall(self.markup:GetHeight() + 4)
	else
		self:SetTall(self.markup:GetHeight())
	end
end
...
function PANEL:AddLine(elements, bShouldScroll, icon)
	...
	if icon then
		buffer[#buffer + 1] = "<font=ixPRIconsChat><color=255,255,255>"..icon.." </color></font>"
	end

	if (CHAT_CLASS) then
		buffer[#buffer + 1] = "<font="
		buffer[#buffer + 1] = CHAT_CLASS.font or "prChatFont"
		buffer[#buffer + 1] = ">"
	end
		...
		elseif (type(v) == "Player") then
			...
			buffer[#buffer + 1] = tostring(v):gsub("<", "&lt;"):gsub(">", "&gt;"):gsub("%b**", function(value)
				...
				if (inner:find("%S")) then
					return "<font=prChatFontItalics>" .. value:utf8sub(2, -2) .. "</font>"
				end
			end)
		end
	end
...
function PANEL:Init()
	...
	self.name:Dock(TOP)
	if ix.option.Get("useAltChatFont", false) then
		self.name:DockMargin(4, 2, 0, 0)
	else
		self.name:DockMargin(4, 2, 0, 0)
	end
	self.name:SetContentAlignment(4)
	self.name:SetFont("prChatFont")
...
function PANEL:SizeToContents()
	...
	if ix.option.Get("useAltChatFont", false) then
		self:SetTall(self.name:GetTall() + (bDescriptionVisible and (self.description:GetTall() + 4) or 8))
	else
		self:SetTall(self.name:GetTall() + self.description:GetTall() + (bDescriptionVisible and -4 or 4))
	end
end
...
function PANEL:AddIconMessage( ... )
	local args = { ... }
	local icon = table.remove(args, 1)
	local class = CHAT_CLASS and CHAT_CLASS.uniqueID or "notice"
	local activeTab = self.tabs:GetActiveTab()
	...
	if (activeTab and !activeTab:GetFilter()[class]) then
		activeTab:AddLine(args, true, icon)
		bShown = true
	end
