...
function ix.squadsystem.CreateSquad(client, squad, iteration)
	local name = table.Random(ix.squadsystem.squadnames)
	if iteration != nil and iteration >= 30 then
		client:Notify("All squad names have been taken!")
	elseif !(ix.squadsystem.squads[name]) then -- Prevents the creation of the squad if it already exists.
		ix.squadsystem.InitializeSquadInfo(client, name)
		...
		ix.squadsystem.squads[name] = {tab}

		ix.squadsystem.SyncSquad(name)

		client:Notify("You have created the squad "..name..'.')
	else
		ix.squadsystem.CreateSquad(client, name, iteration == nil and 1 or iteration + 1)
	end
end
