...
if (CLIENT) then
	...
	function PLUGIN:LoadFonts(font, genericFont)
		surface.CreateFont("ixWeaponSelectHeaderFont", {
			font = "Khand Medium",
			size = ScreenScale(28),
			extended = true,
			weight = 100
		})
		surface.CreateFont("ixWeaponSelectDescFont", {
			font = "Khand Regular",
			size = ScreenScale(12),
			extended = true,
			weight = 100
		})
		surface.CreateFont("ixWeaponSelectSmallHeaderFont", {
			font = "Khand Regular",
			size = ScreenScale(14),
			extended = true,
			weight = 100
	...
	function PLUGIN:HUDPaint()
		if LocalPlayer():GetLocalVar("IsPlacing", false) then return end
		...
		if (fraction > 0.01) then
			...
			for i = 1, #weapons do
				...
				surface.SetFont("ixWeaponSelectHeaderFont")
				...
				cam.PushModelMatrix(matrix)
					ix.util.DrawText(weaponName, 2, ty / 2, color, 0, 1, "ixWeaponSelectHeaderFont")
				cam.PopModelMatrix()
			end
		...
		if (IsValid(weapon)) then
			...
			if (instructions != nil and instructions:find("%S")) then
				...
				text = text .. string.format(
					"<font=ixWeaponSelectSmallHeaderFont><color=%d,%d,%d>%s</font></color>\n%s\n",
					color.r, color.g, color.b, L("Instructions"), instructions
				)
			end

			if (text != "") then
				self.markup = markup.Parse("<font=ixWeaponSelectDescFont>"..text, ScrW() * 0.3)
				self.infoAlpha = 0
			end
