...
function PANEL:Init()
    ...
    self.Tabs.OnActiveTabChanged = function()
        ...
        for k, v in ipairs(self.committedChanges) do
            ...
            entry:Populate(self.target, k, {
                ...
                ["editor"] = ix.char.loaded[dataPoint.editor]:GetName(),
                ["time"] = dataPoint.time,
                ["id"] = k,
                ["data"] = dataPoint.data
            })
    ...
    /*
        for k, v in pairs(iconLegend) do
            local entry = vgui.Create("DataLogEntry", self.Tabs.Logs)
            entry:Populate(self.target, k, {["editor"] = "CCA.C17-UNION.i5.27015", ["time"] = os.time(), ["id"] = math.random(1,100)})
            entry:Dock(TOP)
            entry:DockMargin(5, 5, 5, 5)
        end
        */  
            ...
            self.DWant.OnChange = function(checkbox, checked)
                if checked then
                    self.localChanges[#self.localChanges+1] = {
                        ...
                        ["logType"] = "MakeWant",
                        ["time"] = os.time(),
                        ["editor"] = LocalPlayer():GetCharacter():GetID(), 
                        ["data"] = nil
                    }
                else
                    self.localChanges[#self.localChanges+1] = {
