...
local function RenderNewIcon(panel, itemTable)
	...
	local renderedIcons = ix.data.Get("RenderedIcons", {})
	print("ID", itemTable.uniqueID)
	...
	elseif itemTable.forceRender or !renderedIcons[itemTable.uniqueID] then
		ICON_RENDER_QUEUE[string.lower(model)] = true
		renderedIcons[itemTable.uniqueID] = true

		ix.data.Set("RenderedIcons", renderedIcons)

		panel.Icon:RebuildSpawnIcon()
	end
end

concommand.Add("ix_resetitemicons", function()
	ix.data.Set("RenderedIcons", {})
end)
...
function PANEL:PadHeight(targetHeight)
	local width, height = parent:GetSize()
	local padding = 4
	local iconSize = self:GetIconSize()
	local yPad = (height*(iconSize + padding) - padding)/2

	self:SetTall(targetHeight)
	self:DockPadding(0, yPad, 0, yPad)
end
...
function PANEL:PaintDragPreview(width, height, mouseX, mouseY, itemPanel)
	...
	local padding = self:GetPadding(2)

	local yPad
	if self.padHeight then
		yPad = (self.padHeight - (self.gridH*(iconSize + padding) - padding))/2
		self:SetTall(self.padHeight)
	end
		...
		if (IsValid(hoveredPanel) and hoveredPanel != itemPanel and hoveredPanel.GetItemTable) then
			...
			if (hoveredItem) then
				...
				if (info and info.OnCanRun and info.OnCanRun(hoveredItem, {item.id}) != false) then
					...
					surface.DrawRect(
						hoveredPanel.x,
						(self.padHeight != nil and (yPad + hoveredPanel.y)) or hoveredPanel.y,
		...
		if (itemTable.exRender) then
			...
			panel.ExtraPaint = function(this, panelX, panelY)
				...
				if (exIcon) then
					...
					ikon:renderIcon(
						itemTable.uniqueID,
						itemTable.width * iconSize,
						itemTable.height * iconSize,
						itemTable:GetModel(),
						itemTable.iconCam
					)
				end
...
vgui.Register("prInventory", PANEL, "DFrame")
...
function PANEL:Init()
	self:SetIconSize(math.floor(ScrW()*0.052))
	self:ShowCloseButton(false)
	self:SetDraggable(true)
	self:SetSizable(true)
	self:SetTitle(L"inv")
...
function PANEL:BuildSlots()
	...
	for x = 1, self.gridW do
		...
		for y = 1, self.gridH do
			...
			slot.gridY = y
			if self.padHeight then
				slot:SetPos((x - 1) * iconSize + 4, yPad + (y - 1) * iconSize + self:GetPadding(2))
			else
				slot:SetPos((x - 1) * iconSize + 4, (y - 1) * iconSize + self:GetPadding(2))
			end
			slot:SetSize(iconSize, iconSize)
...
function PANEL:PaintDragPreview(width, height, mouseX, mouseY, itemPanel)
	...
	if (item) then
		local inventory = ix.item.inventories[self.invID]
		local dropX = math.ceil((mouseX - 4 - (itemPanel.gridW - 1) * 32) / iconSize)
		local dropY = (yPad != nil and math.ceil((yPad*-1 + mouseY - self:GetPadding(2) - (itemPanel.gridH - 1) * 32) / iconSize)) or (math.ceil((mouseY - self:GetPadding(2) - (itemPanel.gridH - 1) * 32) / iconSize) ) 
		...
		surface.DrawRect(
			(dropX - 1) * iconSize + 4,
			(self.padHeight != nil and (yPad + (dropY - 1) * iconSize + self:GetPadding(2))) or ((dropY - 1) * iconSize + self:GetPadding(2)),
			itemPanel:GetWide(),
			itemPanel:GetTall()
		)
	end
