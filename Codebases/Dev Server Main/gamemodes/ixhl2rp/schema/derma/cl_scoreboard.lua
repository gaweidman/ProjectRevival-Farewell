...
function PANEL:Init()
	...
	self.nameBar = charInfo:Add("Panel")
	self.titleLbl = self.nameBar:Add("DLabel")
	self.titleLbl:Dock(LEFT)
	self.titleLbl:SetFont("prSmallHeadingFont")
	self.titleLbl:DockMargin(0, 0, 5, 0)
	self.nameLbl = self.nameBar:Add("DLabel")
	self.nameLbl:Dock(LEFT)
	self.nameBar:Dock(TOP)
	self.nameLbl:SetFont("prSmallHeadingFont")
	self.nameBar:DockMargin(10, 6, 0, 0)
	self.nameBar:SizeToContents()
	
	self.descLbl = charInfo:Add("DLabel")
	self.descLbl:Dock(TOP)
	self.descLbl:DockMargin(10, -5, 0, 0)
...
function PANEL:SetPlayer(ply)
	...
	local titles = {
		...
		supervip = "Super VIP",
		gamer = "Gamer",
		epicgamer = "Epic Gamer",
		truegamer = "True Gamer",
		contributor = "Contributor"
	}

	local group = ply:GetUserGroup()
	local title = titles[group]

	if ply:SteamID64() == "76561198066461105" then
		title = "Owner"
	end

	if !title then
		self.titleLbl:Remove()
	else
		if ix.staffgroups[group] > 0 then
			self.titleLbl:SetTextColor(Color(240, 24, 24))
		elseif group == "Contributor" then
			self.titleLbl:SetTextColor(Color(240, 24, 132))
		else
			self.titleLbl:SetTextColor(Color(240, 190, 24))
		end
		self.titleLbl:SetText("["..title.."]")
		self.titleLbl:SizeToContents()
	end
