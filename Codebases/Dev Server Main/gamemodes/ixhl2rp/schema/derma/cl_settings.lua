...
function PANEL:Init()
	...
	self.textEntry:SetFont("prMenuButtonFont")
...
function PANEL:Init()
	...
	self.setting:Dock(RIGHT)
	self.setting:SetFont("prMenuButtonFont")
	self.setting:SetBackgroundColor(Color(65, 65, 65))
...
function PANEL:Init()
	...
	self.text:Dock(LEFT)
	self.text:SetFont("prFieldsMenuThickFont")
...
function PANEL:AddCategory(name)
	...
	if (!IsValid(panel)) then
		...
		panel.paddingTop = 36
		panel:DockPadding(1, panel.paddingTop, 1, 1)

		panel.Paint = function(panel, w, h)
			surface.SetDrawColor(145, 65, 45)
			surface.DrawRect(0, 0, w, panel.paddingTop)
			
			draw.SimpleText(panel:GetText(), "prCategoryHeadingFont", 5, panel.paddingTop/2, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		end
...
function PANEL:SetSearchEnabled(bValue)
	...
	local highlightColor = Color(12, 201, 250)
	...
	self.searchEntry.Paint = function(panel, w, h)
		...
		DisableClipping(true)
			panel:DrawTextEntryText(panel:GetTextColor(), highlightColor, color_white)

			surface.SetDrawColor(97, 97, 97)
			surface.DrawRect(-panel.iconWidth - 4, 0, panel.iconWidth + 4, h)

			surface.SetFont("ixSmallTitleIcons")
			surface.SetTextColor(panel.iconColor)
			surface.SetTextPos(-panel.iconWidth - 2, 0)
			surface.DrawText(panel:GetIcon())
			...
			for category, options in SortedPairs(ix.option.GetAllByCategories(true)) do
				...
				for _, data in pairs(options) do
					...
					row:GetLabel():SetHelixTooltip(function(tooltip)
						...
						title:SetImportant()
						title:SetText(data.phrase or key)
						title:SizeToContents()
...
function PANEL:Init()
	...
	self.nameLbl:SetFont("prMediumMenuFont")
...
function PANEL:SetType(optType)
	if IsValid(self.option) then
		self.option:Remove()
	end

	self.option = self:Add(panelMap[optType])
	self.option:Dock(RIGHT)
	self.option:SetWide(ScrW()/4)

	self.option:SizeToContents()
...
function PANEL:Populate(key, info)
	self.option:Populate(key, info)
end

vgui.Register("prSettingRow", PANEL, "Panel")