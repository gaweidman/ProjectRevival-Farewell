...
function PANEL:Init()
	...
	self.attributes:SetTitle("Choose Your Attributes")
	...
	local attributesProceed = self.attributesPanel:Add("ixMenuButton")
	attributesProceed:SetText("proceed")
	attributesProceed:SetContentAlignment(6)
	attributesProceed:SizeToContents()
	attributesProceed:Dock(BOTTOM)
	attributesProceed.DoClick = function()
		self.progress:IncrementProgress()
		self:SetActiveSubpanel("skills")
	end

	-- skills subpanel
	self.skills = self:AddSubpanel('skills')
	self.skills:SetTitle("chooseSkills")

	local skillsModelList = self.skills:Add("Panel")
	skillsModelList:Dock(LEFT)
	skillsModelList:SetSize(halfWidth, halfHeight)

	local skillsBack = skillsModelList:Add("ixMenuButton")
	skillsBack:SetText("return")
	skillsBack:SetContentAlignment(4)
	skillsBack:SizeToContents()
	skillsBack:Dock(BOTTOM)
	skillsBack.DoClick = function()
		self.progress:DecrementProgress()
		self:SetActiveSubpanel("attributes")
	end

	self.skillsModel = skillsModelList:Add("ixModelPanel")
	self.skillsModel:Dock(FILL)
	self.skillsModel:SetModel(self.factionModel:GetModel())
	self.skillsModel:SetFOV(modelFOV - 13)
	self.skillsModel.PaintModel = self.skillsModel.Paint

	self.skillsPanel = self.skills:Add("Panel")
	self.skillsPanel:SetWide(halfWidth + padding * 2)
	self.skillsPanel:Dock(RIGHT)

	self.skillsPanel:Add("DButton")

	local create = self.skillsPanel:Add("ixMenuButton")
