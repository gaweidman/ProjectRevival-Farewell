...
vgui.Register("nut_lc_Alloying", PANEL, "DFrame")
...
local PANEL = {}
	function PANEL:Init()
		...
		function refreshMolding()
			...
			self.list:SetDrawBackground(true)
			if (self.page == 1) then
				self.moldButton:SetText("Mold")
				self.moldButton:SetEnabled(false)
				self.moldButton:SetTextColor(Color(0,0,0,200))
				self.closeBackButton:SetText("Close")
				self.closeBackButton.DoClick = function()
					self:Remove()
				end

				addCategoryButtons(self.moldableitems)
			elseif (self.page == 2) then
				self.closeBackButton:SetText("Back")
				self.closeBackButton.DoClick = function()
					self.page = 1
				...
				self.moldButton:SetEnabled(false)
				self.moldButton:SetTextColor(Color(0,0,0,200))
				self.moldButton.DoClick = function()
					netstream.Start("nut_lc_MoldItem", self.itemSelected.uniqueID, self.caller)
				end
				addItemButtons(self.categorySelected)
			end
		end
		...
		function addCategoryButtons(categories)
			...
			if categories and table.getn(categories) > 0 then
				...
				self.youHaveNo = vgui.Create("DLabel", self)
				local t = "You don't have objects to mold"
				self.youHaveNo:SetText(t)
				local width, heigth = surface.GetTextSize(t)
				self.youHaveNo:SetSize(width,heigth)
				self.youHaveNo:SetPos( ((0.78630136986301 * self:GetWide()) - (0.01369863 * self:GetWide()) - (0.2 * self:GetWide()))/2 + (self.youHaveNo:GetWide()/2), 0.91625615763547 * self:GetTall() )
		...
		function addItemButtons(category, itemSel)
			...
			if category then
				for k, v in SortedPairs(nut.item.list) do
					local id = v.uniqueID
					if (table.HasValue(self.moldableitems, id)) then
						local cat = v.category
						if (cat == category) then
							table.insert(items, v)
						end
			...
			for _, v in pairs(items) do
				...
				creatingItemButton.DoClick = function()
					...
					self.moldButton:SetEnabled(true)
					self.moldButton:SetTextColor(Color(255,255,255,255))
					creatingItemButton:SetEnabled(false)
					name:SetTextColor(Color(255,255,255,255))
					desc:SetTextColor(Color(255,255,255,255))
					self.oldButton = self.itemsButtons[v.name]
		...
		self.moldButton = vgui.Create( "DButton", self )
		self.moldButton:SetEnabled(false)
		self.moldButton:SetPos( 0.01369863 * self:GetWide(), 0.91625615763547 * self:GetTall() )
		self.moldButton:SetSize( 0.2 * self:GetWide(), 0.054187192118227 * self:GetTall() )
		self.moldButton:SetText("Mold")
		self.moldButton:SetTextColor(Color(255,255,255,255))
...
vgui.Register("nut_lc_Molding", PANEL, "DFrame")
...
vgui.Register("nut_lc_Casting", PANEL, "DFrame")