...
function PLUGIN:showAlloyableItems(client)
	local data = client:getChar():getData("alloyableItems")
	if !data then
		client:getChar():setData("alloyableItems", {})
	end
	return data
end

function PLUGIN:showMoldableItems(client)
	local data = client:getChar():getData("moldableItems")
	if !data then
		client:getChar():setData("moldableItems", {})
	end
	return data
end

function PLUGIN:showCastableItems(client)
	local data = client:getChar():getData("castableItems")
	if !data then
		client:getChar():setData("castableItems", {})
	end
	return data
end
...
local function canPlayerMold(client, item)
	local inventory = client:getChar():getInv()
	local items = inventory:getItems()
	local recipe = copyTable(item.recipe) --So that the original table isn't erased for the item
	local tools = item.tools
...
local function canPlayerAlloy(client, item)
	local inventory = client:getChar():getInv()
	local items = inventory:getItems()
	local recipe = copyTable(item.recipe) --So that the original table isn't erased for the item
	local tools = item.tools
...
local function canPlayerCast(client, item)
	local inventory = client:getChar():getInv()
	local items = inventory:getItems()
	local recipe = copyTable(item.recipe) --So that the original table isn't erased for the item
	local tools = item.tools
...
function alloy(client, item)
	local recipe = item.recipe
	if (give(client,item)) then
		for k, v in pairs(recipe) do
			remove(client:getChar():getInv(), k, v)
		end
		client:notifyLocalized("lc_youAlloyed", item.name)
		return true
	else
		client:notifyLocalized("lc_cantFit", item.name)
		return false
...
function cast(client, item)
	local recipe = item.recipe
	if (give(client,item)) then
		for k, v in pairs(recipe) do
			remove(client:getChar():getInv(), k, v)
		end
		client:notifyLocalized("lc_youMolded", item.name)
		return true
	else
		client:notifyLocalized("lc_cantFit", item.name)
		return false
...
netstream.Hook("nut_lc_AlloyItem", function(client, item, machine)
	local itemEntity = getItemEntity(item)
	if !itemEntity or itemEntity == nil then
		ErrorNoHalt("@lc_corrupt")
	end
	local canCraft, item = canPlayerAlloy(client, itemEntity)
	if (canCraft == nil) then
		client:notifyLocalized("lc_noRecipe", item.name)
		return false
	end
	...
	if (canCraft) then
		return alloy(client, item)
	end
	client:notifyLocalized("lc_youHaventRecipe", item.name)
	return false
end)

netstream.Hook("nut_lc_CastItem", function(client, item, machine)
	local itemEntity = getItemEntity(item)
	if !itemEntity or itemEntity == nil then
		ErrorNoHalt("@lc_corrupt")
	end
	local canCast, item = canPlayerCast(client, itemEntity)
	if (canCast == nil) then
		client:notifyLocalized("lc_noRecipe", item.name)
		return false
	end
	...
	if (canCast) then
		return cast(client, item)
	end
	client:notifyLocalized("lc_youHaventRecipe", item.name)
	return false
end)

netstream.Hook("nut_lc_MoldItem", function(client, item, machine)
	local itemEntity = getItemEntity(item)
	if !itemEntity or itemEntity == nil then
		ErrorNoHalt("@lc_corrupt")
	end
	local canMold, item = canPlayerMold(client, itemEntity)
	if (canMold == nil) then
		client:notifyLocalized("lc_noRecipe", item.name)
		return false
	end
