...
function COMMAND:OnRun(player, arguments)
	...
	if (target) then
		...
		if (channelTable) then
			...
			local channelList = target:GetCharacterData("listen_channels", {});
			...
			if (channelList[channelID]) then
				...
				if (player != target) then
					Clockwork.player:Notify(player, "You have subscribed "..target:Name().." to '"..channelTable.name.."'.");
					Clockwork.player:Notify(target, player:Name().." has subscribed you to '"..channelTable.name.."'.");
				else
					Clockwork.player:Notify(player, "You have been subscribed to '"..channelTable.name.."'.");
				end;
			end;

			target:SetCharacterData("listen_channels", channelList);
			target:SetChannels();
		else
