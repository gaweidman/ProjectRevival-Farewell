...
if (SERVER) then
	function ENT:SpawnFunction(client, trace)
		local dispenser = ents.Create("ix_cidprinter")
	...
	function ENT:Initialize()
		...
		self.dummy:SetModel("models/dorado/tarjeta2.mdl")
	...
	function ENT:SpawnRation(callback, releaseDelay)
		...
		local itemTable = ix.item.Get("cid")
		...
		timer.Simple(releaseDelay, function()
			ix.item.Spawn("cid", self.dummy:GetPos(), function(item, entity)
				self.dummy:SetNoDraw(true)
			end, self.dummy:GetAngles(), {
				description = string.format("A citizen identification card with ID #%s, assigned to %s.", couNum, couName), 
				id = couNum,
				name = couName
			}
	...
	function ENT:Use(client)
		...
		if (true) then
			...
			cou = client:GetCharacter():GetInventory():HasItem("relocationcoupon")
			couNum = cou:GetData("id", "00000")
			couName = cou:GetData("name", "herpes")
