	...
	local MODES = {
		...
		},
		{
			function(client)
				...
				if (character and (character:GetInventory() and (!character:GetInventory():HasItem("cid") or (character:GetFaction() == FACTION_CAC)))) then
					return true
				else
					return false
				end
	...
	hook.Add("ShouldCollide", "ix_forcefields", function(a, b)
		...
		if (IsValid(entity) and entity:GetClass() == "ix_forcefield") then
			if (IsValid(client)) then 
				if (client:HasBiosignal() or
					character:GetFaction() == FACTION_ADMIN or
					(character:GetFaction() == FACTION_CAC) or
					character:GetFaction() == FACTION_OSA or
					character:GetFaction() == FACTION_CONSCRIPT) then
					return false
				end
