...
ENT.Type = "anim"
ENT.PrintName = "Hazmat Forcefield"
ENT.Category = "HL2 RP"
ENT.Spawnable = true
ENT.AdminOnly = true
...
if (SERVER) then
	function ENT:SpawnFunction(client, trace)
		...
		local entity = ents.Create("ix_testent1")
	...
	function ENT:Initialize()
		self:SetModel("models/hlvr/combine_hazardprops/combinehazardprops_forcefield_right.mdl")
		...
		self.dummy:SetModel("models/hlvr/combine_hazardprops/combinehazardprops_forcefield_left.mdl")
		...
		local verts = {
			{pos = Vector(0, 0, -25)},
			{pos = Vector(0, 0, 150)},
			{pos = self:WorldToLocal(self.dummy:GetPos()) + Vector(0, 0, 0)},
	...
	function ENT:Use(activator)
		...
		if (activator:HasBiosignal()) then
	...
	hook.Add("ShouldCollide", "ix_forcefields", function(a, b)
		...
		if (IsValid(entity) and entity:GetClass() == "ix_forcefield") then
			if (IsValid(client)) then
				if (client:HasBiosignal() or client:Team() == FACTION_ADMIN) then
					return false
				end
	...
	function ENT:DrawShield(vertex)
		mesh.Begin(MATERIAL_QUADS, 1)
			...
			mesh.Position(self:GetUp() * 50)
			mesh.TexCoord(0, 0, 3)
			mesh.AdvanceVertex()

			mesh.Position(vertex + self:GetUp() * 50)
			mesh.TexCoord(0, 3, 3)
			mesh.AdvanceVertex()
