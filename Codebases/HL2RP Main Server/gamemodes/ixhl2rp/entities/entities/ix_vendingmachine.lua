...
ENT.Items = {
	...
	{"CHIPS", "uuchips", 5},
	{"FILTER", "citizenfilter", 2}
}

function ENT:GetStock(id)
	return 5 --self:GetNetVar("stock", {})[id] or self.MaxStock
end
...
if (SERVER) then
	function ENT:Initialize()
		self:SetModel("models/projectrevival/props_interiors/vendingmachinesoda01a.mdl")
	...
	local glowSprite = Material("sprites/light_glow02_add_noz")
	...
	function ENT:Draw()
		...
		for i = 8, 1, -1 do
			...
			local pos = position + forward * 17.33 + right * -31.54 + up * -26.91 + (x + width + 6 + smallWidth/2)*0.06*right + (y + height/2)*0.06*up
