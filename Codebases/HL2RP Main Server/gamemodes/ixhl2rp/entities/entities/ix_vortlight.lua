...
function ENT:Think()
    if CLIENT then
        ...
        if parentEntIndex != nil then
            ...
            if localPlayer:GetViewEntity():EntIndex() == localPlayer:EntIndex() then
                self.pos = self.pos or parent:GetPos()
                self.projPos = parent:GetPos()

                self.pos = Lerp( 1, self.pos, self.projPos )
                --self:SetPos( self.pos + Vector(999999, 999999, 999999)) -- This is so the light isn't drawn in first person.
                lightpos = self.pos + Vector(0, 0, 42)
            else
