...
if CLIENT then
	...
	surface.CreateFont("ixCMBSquadFont", {
		font = "Courier New",
		size = 18,
		extended = false,
		weight = 550,
	...
	function PLUGIN:HUDPaint()
		for k, v in pairs(squad) do
			if (v and v.member and (v.member != LocalPlayer() or true ) and IsValid(v.member)) then
				...
				local memberName = squad.name.."-"..k

				if k == 1 then
					draw.SimpleTextOutlined( "★", "ixCMBSquadFont", screenpos.x, screenpos.y, v.color, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black )
				else
					draw.SimpleTextOutlined( "⮟", "ixCMBSquadFont", screenpos.x, screenpos.y, v.color, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black )
				end
				draw.SimpleTextOutlined( memberName, "ixCMBSquadFont", screenpos.x, screenpos.y-20, v.color, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black )
			end
		end
	end
else
