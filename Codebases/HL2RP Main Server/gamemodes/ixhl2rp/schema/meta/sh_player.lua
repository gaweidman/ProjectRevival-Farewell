...
function playerMeta:HasClassWhitelist(class)
	...
	if (data) then
		...
		print(Schema.folder, ix.faction.Get(data.faction).uniqueID, data.uniqueID)
		return (ixData[Schema.folder] and ixData[Schema.folder][ix.faction.Get(data.faction).uniqueID] and ixData[Schema.folder][ix.faction.Get(data.faction).uniqueID][data.uniqueID]) or false
	end
...
function playerMeta:TryFreeVort(inflictor)
	...
	if hasCombineQualifs or inflictorChar:GetSkill("medical", 0) < 30 then
...
function playerMeta:IsStaff()
	...
	return ix.staffgroups[userGroup] > 0
end

function playerMeta:IsStaffRank(checkRank)
	local userGroup = self:GetUserGroup()

	return ix.staffgroups[checkRank] <= ix.staffgroups[userGroup]
end