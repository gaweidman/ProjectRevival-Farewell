...
ITEMS.aptkey = {
	...
	["description"] = "A golden key with a",
	["width"] = 1,
	["height"] = 1
}
...
for k, v in pairs(ITEMS) do
	...
	ITEM.category = v.category or "Roleplay Items"
	...
	if k == "aptkey" then
		function ITEM:GetDescription()
			if self then
				local building = self:GetData("building")
				local apartment = self:GetData("apartment")

				if !building or !apartment then
					return "A golden blank key with no teeth."
				else
					return string.format("A golden key with the text %s %s engraved on its handle.", building, apartment)
				end
			else
				return "A golden blank key with no teeth."
			end
